...
caminho:=ExtractFilePath(Application.ExeName);
  ...
  if Frm_pesq_fornecedor.frxReport1.LoadFromFile(caminho + 'REL_FORNECEDOR.fr3') then
  begin


    frxReport1.clear; // limpa relatorio
    frxReport1.LoadFromFile(extractfilepath(application.ExeName) + 'REL_FORNECEDOR.fr3');
...
Q_pesq_padrao.Sql.Clear;          //limpa sql
    ...
    Q_pesq_padrao.sql.add('SELECT FORNECEDOR_ID, '
    + 'NOME, '
    + 'ENDERECO, '
    + 'NUMERO, '
    + 'BAIRRO,'
    ...
    0: begin
      //pesquisa por codigo
     Q_pesq_padrao.sql.add('WHERE FORNECEDOR_ID =:PFORNECEDOR_ID');           // criamos parametro
